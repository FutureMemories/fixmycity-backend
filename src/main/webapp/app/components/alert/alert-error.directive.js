(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('fixmycityApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-fixmycityApp-error');
                var entityKey = httpResponse.headers('X-fixmycityApp-params');
                if (errorHeader) {
